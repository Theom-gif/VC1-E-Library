...
type AuthGateProps = {
  children: (props: {
    user: SessionUser;
    logout: () => void;
    login: (payload: {email: string; password: string; role?: RoleName; remember?: boolean}) => Promise<void>;
    register: (payload: {
      firstname: string;
      lastname: string;
      email: string;
...
async function loginWithFallbacks(payload: {email: string; password: string; role?: RoleName; remember?: boolean}) {
      ...
      ? [
          {
            ...
            role: payload.role,
            role_id: roleIdFromRole(payload.role),
            remember,
            remember_me: remember,
          },
          {email: payload.email, password: payload.password, role: titleRole(payload.role), remember, remember_me: remember},
          {email: payload.email, password: payload.password, role_id: roleIdFromRole(payload.role), remember, remember_me: remember},
        ]
      : []),
    {email: payload.email, password: payload.password, remember, remember_me: remember},
  ];
...
export default function AuthGate({children}: AuthGateProps) {
  ...
  useEffect(() => {
    ...
    const tokenAtBoot = authService.getToken();
    let alive = true;
    ...
    void authService
      ...
      .catch((requestError: any) => {
        ...
        // If we don't have a stored token, this 401 may be caused by missing credentials
        // (e.g. cookie-based auth on a different origin). Don't force logout on refresh.
        if (!tokenAtBoot) return;
  ...
  const login = async (payload: {email: string; password: string; role?: RoleName; remember?: boolean}) => {
    const remember = payload.remember !== false;
    ...
    const user = toSessionUser(meResponse || response, payload.email, payload.role ?? 'user', previousMemberSince);
    saveSession(user);
    setAuthRequired(false);
    setForceLogin(false);
    setSessionUser(user);
  ...
  const register = async (payload: {
    ...
    const user = toSessionUser(meResponse || sessionSource, payload.email, payload.role, new Date().toISOString());
    saveSession(user);
    setAuthRequired(false);
    setForceLogin(false);
    setSessionUser(user);
      ...
      <div className="flex w-full max-w-[980px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-[#16282b] dark:shadow-2xl">
        ...
        <div className="w-full bg-white p-8 lg:w-1/2 lg:p-12 dark:bg-[#16282b]">
          ...
          <form className="space-y-5" onSubmit={onLogin}>
            <div><label className="mb-2 block text-sm font-medium text-slate-500 dark:text-[#94a3b8]">Email / Gmail</label><div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-[#94a3b8]" size={18} /><input type="email" value={loginForm.email} onChange={(event) => {setLoginForm((prev) => ({...prev, email: event.target.value})); setError('');}} placeholder="name@gmail.com or you@example.com" className="w-full rounded-xl border border-slate-200 bg-[#f8fbfc] py-3 pl-12 pr-4 text-slate-900 focus:border-[#5da7b3] focus:outline-none dark:border-white/10 dark:bg-[#1d3438] dark:text-[#f8fafc]" required /></div></div>
